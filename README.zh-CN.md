...
一个常驻后台的 agent，像教务老师那样读懂大学的学位规则。一切正常时它**完全沉默**，
只在"再不出声学生就要付出无法挽回的代价"时才冒泡——而且**当场把事做掉**，不是描述一遍。
...
先修课链条，要等到两个学期后才咬人。一笔图书罚款，悄悄升级成注册封锁。一条规则变更，
埋在手册的某个内页，而不是发进你的邮箱。一份学位计划，必须在选课窗口开放前审批通过，
顺序没人解释过。

这些都不是秘密，全部白纸黑字公开着。但它是**按法典的方式公开的**——完整、没有索引、
写给执行它的人看。

被这套规则逮住的，不是学业吃力的学生，而是**家里没有人走过这条路**的学生。第一代大学生。
国际学生——他们除了学期日历，还要额外对着一张签证日历。这就是本项目的全部受众，
...
**选课系统恰恰就是这句话要排除的东西。** Compass 不是一个学生主动打开的 App，
而是一个持续读规则的守夜人——**一学期最多打扰你六次**，而每一次不打扰的代价，
都是一件她收不回来的事。
...
| **后果推理** | 读出一个截止日期，返回结构化 `Finding`：`{reversible, days_until_last_safe_action, confidence, options}` | Strands + Pydantic v2 | ✅ |
| **真实副作用** | 七个动作，按教务处系统的方式拒绝——先修课未过、学分上限、课已满、审核不过 | `src/compass/tools/actions.py` | ✅ |
| **可复现的回执** | 每个动作写一条 append-only 回执，确认号是**确定性**推导的。重跑 demo，同一串号 | `data/receipts.jsonl` | ✅ |
...
安装就这些。没有数据库、没有云依赖、demo 跑起来之前不需要配置任何凭证。
...
`crt` extra 时会拒绝使用该 provider。如果你的凭证是普通 access key，则不需要装这个。

架构图已经签入仓库，所以跑项目、读项目都不需要 `docs` extra——只有重画它才需要。
...
`agentcore invoke` 会把你给的一切包成 `{"prompt": ...}`，所以只能到达契约里"问答"那一半，
` sweep` 和 `decide` 永远收不到真正的 payload。另一个动词需要 payload 完整抵达——
...
│   └── receipts.jsonl         它做过的一切，append-only
│
├── src/compass/
...
你问模型*"这个重要吗？"*，它会说是。它几乎对什么都说是，因为"有点重要"永远站得住脚。
所以所有这样搭起来的通知系统最后都变成噪音——而一个已经学会无视自己 agent 的学生，
**比根本没有 agent 更惨**：她连那份会让她主动去看一眼的担心都失去了。

**Compass 从不问模型要不要出声。**

模型负责建立**事实**——发生了什么变化、代价是多少、最后一个安全时刻在什么时候、
它有多确信、有哪些选项。这些以 `Finding` 的形式交上来。而"要不要打扰人"这个决定，
由 [`src/compass/gate.py`](src/compass/gate.py) 里纯粹的 Python 按固定顺序做出：
...
- **可解释。** 每个判决都引用产生它的那条规则，用写给学生的语言，并直接显示在卡片上。
  她可以问*"我为什么会看到这个？"*，并得到真实答案。
- **可复现。** 同一条 finding 在同一天永远得到同一个判决。所以录下来的 demo
  和测试套件验证的是同一件事。
- **可审计。** 沉默也带理由。无论是否冒泡，`GateDecision.reason` 都有值——
  于是*"它什么都没做"*是可检视的，而不是跟"它崩了"长得一样。
...
每一个都**按真实系统的方式拒绝**。在先修课未通过的情况下注册 *Econometrics II*，
返回的是：
...
注册会检查未满足的先修课、学期 ECTS 上限、余位、重复注册，并且**在任何改动发生之前**
完成全部校验——所以一次被拒绝的注册不会占掉座位。`file_degree_plan` 会拒绝一份
通不过自身审核的计划，正是这让"先修复、再提交"变成强制顺序，而不是建议。

每一个成功的动作都写一条回执，确认号是**确定性**的——由动作及其参数推导，
而不是随机值，所以重跑 demo 会得到同一串号。**回执就是证据**：
`data/receipts.jsonl` 是它做过的一切的 append-only 记录，**包括那些没问就做了的事**。

### Compass 不会做的事

用限制来表述，因为这样的系统是由它拒绝做的事定义的：

- **不会花学生的钱。** 任何有成本的动作都是一张卡，永远不是自动执行。
- **不会自作主张改变她学什么。** 转专业只会被"提供"，不会被"执行"。
- **不会替她联系人。** 除非她主动选择。给导师发消息的不可逆程度和修一份计划不同，
  回执里写明了这一点。
- **不会凭低置信度的判断行动。** 低于 0.70 一律沉默，无论猜对了后果多严重。
- **不会为一件以后还能修的事打扰她。** 可挽回的问题在后台处理，直到它不再可挽回。
...
下一个来看的人就没有东西可看了。生成器是确定性的、数据是虚构的，所以把它放回去
...
全新数据集，并且有一个 autouse fixture 会把 AWS 和 provider 凭证从环境里剥掉，
这样套件在一台恰好登录着的机器上不会表现不同。
...
- **[`test_bundle.py`](tests/test_bundle.py)** —— 在临时目录里搭出部署包的目录形状，
  ...
  的修复会通过所有本地测试，却在第一次真实调用时失败。**
  这是再多常规本地测试也找不出来的部署 bug。**

  断言落在**入口为子进程构建的环境**上，而不是落在集成上：把 `PYTHONPATH` 那行导出
  删掉再跑集成检查，**它照样通过**，因为子进程以正常的 `site` 处理启动，
  而 checkout 里的 editable 安装反正也能解析这个 import。**
  一个把修复删掉还通过的测试不是测试**，所以不变量被断言在它真正承重的地方，
  集成那一半被明确标注为它本来的样子：冒烟测试。
...
意味着发布一份**对真实机构规章作出断言**的文档。那些断言会在该机构改规则的那一刻
变成错的，而且错的方式可能误导一个真实的学生。
...
**要泛化的是规则形状，不是一个专业。** 目前的规则词汇表只有一个经济学专业。
值得回答的问题是那些*形状*能不能泛化——我认为能，因为生成器本来就按 requirement
group 参数化，而闸门对"经济学"一无所知。

**把没写下来的规则也读进来。** Sentinel 现在推理的是手册上写着的规则。
更难、也更有价值的那版，推理的是**没写下来的那部分**——那种
*"学院可酌情决定"*的句式。

**从服务一个学生到服务一群人。** 目前的 demo 只跟一份记录。
闸门、白名单、回执日志本来就都是按学生隔离的，而**回执日志恰恰是学生支持办公室
真正会想要的那块东西**：一份"agent 自己做了什么、为什么"的审计留痕。

**从定时扫描到事件驱动。** 现在的扫描是按需触发的；一个跑满整个学期的部署
应该挂在校历触发器上，以及手册变更检测上。
